...
export const useMemberStore = defineStore('members', () => {
    ...
    const [t, d] = await Promise.all([
      applyDate(
        supabase.from('takeout_orders')
          ...
          .eq('customer_phone', clean).eq('status', 'done')
  ...
  function exportExcel() {
    const rows = members.value.map(m => ({
      ...
      '最後消費': m.lastVisit      ?? '',
      '備註':     m.lastNote       ?? '',
      '黑名單':   m.isBlacklisted ? '是' : '',
      '黑名單原因': m.blacklistReason ?? '',
    }))

    const ws = XLSX.utils.json_to_sheet(rows)
    ws['!cols'] = [
      { wch: 12 }, { wch: 14 }, { wch: 14 }, { wch: 35 },
      { wch: 10 }, { wch: 12 }, { wch: 20 }, { wch: 8 }, { wch: 20 },
    ]
  ...
  return {
    ...
    init, findByPhone, upsertMember, updateContact,
    deleteMembers, fetchMemberOrders,
    recordArrival, setBlacklist,
    exportExcel,
  }
})